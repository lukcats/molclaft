...
![MolCLaFT图](Snipaste_2026-07-10_14-12-37.png)
...
This copy also includes a MolCLaFT implementation adapted from T5:

- `molclaft/pretraining.py`: multi-objective training with masked component reconstruction (MCR) and structured denoising (SDN).
- `molclaft/captioning.py`: in-context molecule captioning with Morgan fingerprint Dice-similarity retrieval and the auxiliary contextual mapping loss.
- `molclaft/modeling_molclaft.py`: cross-modal toxicity model with a frozen T5/CLM encoder, trainable GIN graph encoder, contrastive feature alignment, class-level distillation, and final toxicity prediction.
- `train_molclaft_captioning.py`: ICMC fine-tuning entry point.
- `train_molclaft_toxicity.py`: toxicity prediction entry point. The input file should contain a SMILES column and a label column, for example `SMILES` and `label`.

Example toxicity training command:

```bash
python train_molclaft_toxicity.py \
  --train_file path/to/train.tsv \
  --validation_file path/to/valid.tsv \
